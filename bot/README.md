...
- Подбор свободных слотов по рабочим часам и занятости с двойной проверкой: локальные записи в `appointments` + занятые интервалы из Google Calendar специалиста (ключ берется из связанного `web_users` через `specialist_identity_links`, либо из legacy-полей `specialists`).
...
- Для интеграции календаря у специалиста используется связка:
  ...
  - `specialists.google_api_key` / `specialists.google_calendar_id` как fallback для обратной совместимости

...
- При переносе записи бот синхронизирует время существующего события в Google Calendar (по `appointmentId` в `extendedProperties`; для старых событий есть fallback-поиск по описанию).
...
- Для клиента хранится `clients.timezone` (IANA): если timezone не известна, используется fallback timezone аккаунта; это значение применяется в пользовательских сообщениях/напоминаниях для корректной локальной даты/времени.
- Логика хранения времени остается UTC (в БД), а вычисления доступности и отображение выполняются через timezone-слой аккаунта/клиента.

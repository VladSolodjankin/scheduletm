...
- Подбор свободных слотов по рабочим часам и занятости с двойной проверкой: локальные записи в `appointments` + занятые интервалы из Google Calendar специалиста (если у специалиста заданы `google_api_key` и `google_calendar_id`).
...
- Для интеграции календаря у специалиста можно задать:
  - `specialists.google_api_key`
  - `specialists.google_calendar_id`

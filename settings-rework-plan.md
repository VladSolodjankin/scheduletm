...
11. Server: реализована авто-отмена неоплаченных записей через scheduler/job:
   - выборка `unpaid + active` записей-кандидатов;
   - проверка `specialist_booking_policies.auto_cancel_unpaid_enabled`;
   - отмена при достижении `created_at + unpaid_auto_cancel_after_hours`;
   - запись audit причины `auto_cancel_unpaid`;
   - покрытие unit-тестами job-логики.
12. Закрыта сквозная UX-поддержка поздней отмены (refund/no-refund) в web + bot:
   - web: перед подтверждением отмены показывается итог по возврату;
   - bot: добавлен шаг подтверждения отмены с явным текстом refund/no-refund;
   - bot: после отмены отправляется итоговый статус по возврату в пользовательском сообщении.

### ⏭️ Следующая итерация

1. Доработка отправки оповещений (см. чеклист ниже: реальные провайдеры каналов, retries, observability).
2. (закрыто) Полная сквозная UX-поддержка последствий поздней отмены (refund/no-refund) в web и bot.

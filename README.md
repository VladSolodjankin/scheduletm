...
- Web: добавлена страница `/appointments`:
  ...
  - режимы просмотра `Day/Week/Month`: week-view стартует с понедельника, текущий день подсвечивается в шапке, а month-view показывает список записей по каждой дате;

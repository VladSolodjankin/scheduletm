...
- Integrations: Google OAuth start/callback, Telegram bot token в user integrations. Google connect action in `Integrations` uses a branded Google button with G icon for clearer provider identification. Telegram username and bot token fields in `User settings` explicitly disable browser credential autofill (`off` and `telegram-bot-token`) to avoid accidental login/password substitution when fields are empty.

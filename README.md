...
- Базовый контур напоминаний: планирование уведомлений в `notifications`, обработка фоновой джобой с ретраями и каналами Telegram/email/SMS (email/SMS пока stub-провайдеры); при переносе записи напоминания пересоздаются, при отмене переводятся в `cancelled`.

...
UI auth-хелпер кеширует `storageState` после первого успешного входа для каждой пары email/password и переиспользует его в следующих тестах, чтобы уменьшить количество повторных логинов в рамках одного запуска Playwright.

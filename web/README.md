...
```bash
VITE_API_URL=https://www.meetli.cc
```

Если `VITE_API_URL` не задан, клиент теперь использует текущий origin браузера (`window.location.origin`), и только потом fallback на `http://localhost:3000` для локальной разработки.

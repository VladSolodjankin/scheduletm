...
export function AddBlockDialog({ open, locale, usedPlatforms, onClose, onConfirm }: {
  open: boolean; locale: Locale; usedPlatforms: ReadonlySet<SocialPlatform>; onClose: () => void; onConfirm: (block: PageBlock) => void;
}) {
  ...
  return <>
    <Dialog open={open && !draft} onClose={close} fullWidth maxWidth="md">
      ...
      <DialogContent dividers>
        {!category ? <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr', sm: 'repeat(3, 1fr)' }, gap: 2 }}>
          {(Object.keys(categories) as Category[]).map((item) => <Paper key={item} component="button" type="button" onClick={() => setCategory(item)} sx={tileSx}>
            <Typography sx={{ fontWeight: 700 }}>{publicPageText(locale, item)}</Typography>
          </Paper>)}
          {getBlockDefinitions().filter(({ type }) => type !== 'hero' && type !== 'social-button').map((definition) => {
            ...
            return <Paper key={definition.type} component="button" type="button" onClick={() => setDraft(createBlock(definition.type))} sx={tileSx}>
              <Typography sx={{ fontWeight: 700 }}>{labelKey ? publicPageText(locale, labelKey) : definition.name}</Typography>
            </Paper>;
          })}
    ...
    <BlockEditorDialog open={Boolean(draft)} block={draft} locale={locale} title={publicPageText(locale, 'configureBlock')}
      onClose={() => setDraft(null)} onSave={({ block }) => { onConfirm(block); close(); }} />
  </>;
}

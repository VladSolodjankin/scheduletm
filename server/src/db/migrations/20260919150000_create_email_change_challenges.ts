...
const TABLE = 'email_change_challenges';
...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable(TABLE, (table) => {
    ...
    table.string('new_email', 254).notNullable();
    ...
    table.index(['web_user_id', 'created_at'], 'email_change_challenges_user_created_idx');
    table.index(['web_user_id', 'consumed_at'], 'email_change_challenges_user_consumed_idx');
  });
}

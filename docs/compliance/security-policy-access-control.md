...
- System-level settings are restricted to the `product_admin` role.
- Account management is restricted to `product_admin` and the tenant `owner`.
...
- `product_admin`: global platform administrative scope.

...
| OAuth scope minimality (Zoom) | `oauth-scope-and-token-handling.md` | Reviewed, compliant | Only `meeting:write:meeting`/`user:read:user` requested; matches what's actually registered/approved in the Zoom console. A prior mismatch (code default requested the unregistered `meeting:read:meeting` instead of `user:read:user` if the env var was ever unset) was found and fixed on 2026-09-21. |

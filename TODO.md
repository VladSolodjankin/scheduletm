...
- [x] Tenant RBAC: `owner` управляет только своим пространством.
- [x] Глобальная роль `product_admin` с доступом ко всем аккаунтам; роль нельзя назначить через обычный Users API/UI.

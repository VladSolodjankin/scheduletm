...
- [x] Добавить i18n словари в `server` для ответов API (`ru/en`) с выбором локали из запроса.
...
- [x] Локализовать таблицу и форму страницы `/users` через web i18n словари (`ru/en`).
- [x] Перевести поле телефона в форме пользователя на переиспользуемый `AppRhfPhoneField`.
